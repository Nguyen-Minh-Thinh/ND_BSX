...
def video_playback(source,firebasers):
    cap = cv2.VideoCapture(source)
    while True:
        ret, frame = cap.read()
        frame = cv2.resize(frame,dsize=(1280 ,720))
        # print(ret)
        firebase.put('/bike/',firebasers,ret)
        # print(frame)
        # frame = cv2.flip(frame, 1)
        img = findLP_img(frame)
        # cv2.imshow('img', frame)
...
def ipCam(ipCamAdd, port):
    while True:
        ...
        img_res = requests.get("http://" + ipCamAdd + ":" + port + "/shot.jpg")
...
def modifiedMainWithIPCam(ipCamAdd, portNum):
    #Tìm biển số
    img_res = requests.get("http://" + ipCamAdd + ":" + portNum + "/shot.jpg")
    print("http://" + ipCamAdd + ":" + portNum + "/shot.jpg")
    img_arr = np.array(bytearray(img_res.content), dtype = np.uint8)
...
def getVese(firebase):
    bikelistdatain= firebase.get('/bikelist/','')
    bikelistrs=str(bikelistdatain)
    bikelistkey=bikelistrs.split('\'')
    bikelistkey=bikelistkey[1]
    bikelistrs=bikelistrs.split('[')
    bikelistrs=bikelistrs[1].split(']')
    bikelistrs=bikelistrs[0]
    bikelistrs=bikelistrs.replace('\'',"")
    bikelistrs=bikelistrs.replace(' ',"")
    bikelistrs=list(bikelistrs.split(","))
    vese=len(bikelistrs)-1
    return vese
...
def getBoolean(firebase):
    boolean= firebase.get('/boolean/','')
    boolean=str(boolean)
...
def getBikekey(firebase):
    bikekey=firebase.get('/bike/','')
    bikekey=str(bikekey)
    bikekey=bikekey.split('\'')
    bikekey=bikekey[1]
    return bikekey

def ClientHandling(firebase):
    ipCamAdd = input("Nhập ip camera:")
    portCam = input("Nhập port:")
    # threadCam = threading.Thread(target=ipCam, args=(ipCamAdd, portCam, ))
    # threadCam.start()
    while True:
        # print("client runnning")
        if keyboard.is_pressed('e'):
            # OriImg = cv2.imread('./Bike_back/xm7nho.jpg',1)
            key=input("Nhập key: ")
            # pic(OriImg,getBikekey(firebase),key)
            # numplate=input("Nhập biển số: ")
            numplate = modifiedMainWithIPCam(ipCamAdd, portCam)
            print("Biển số:" + numplate)
            check = False
            while not check:
                correct = input("Biển số đã chính xác chưa?y/n")
            ...
            if numplate == "":
                ...
                continue
            firebase.put('/bike/',getBikekey(firebase),numplate+key)
            while True:
                print("Doi kiem tra tren he thong")
                status= getBoolean(firebase)
                if status == "2":
                    print("Thêm xe thành công")
                    firebase.put('/boolean/',getBooleanKey(firebase),"")
                    break
                elif status == "1":
                    print("Có thể cho xe ra")
                    firebase.put('/boolean/',getBooleanKey(firebase),"")
                    break
                elif status == "0":
                    print("sai key")
...
if __name__ == "__main__":
    ...
    # video_webcam()
    camIP = input("Camera IP:")
    port = input("Camera port:")
    ipCam(camIP, port)

    # firebase = firebase.FirebaseApplication('https://ptit-htpt.firebaseio.com/', None)
